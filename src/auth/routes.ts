...
import { type AppSessionStore } from "./atproto-oauth.js";
 ...
 */
export function createAuthRoutes({ oauthClient, appSessionStore, publicUrl }: AuthRouteOptions): Hono {
	...
	routes.get("/callback", async (c) => {
		try {
			const { session } = await oauthClient.callback(new URL(c.req.url).searchParams);

			const appSession = await appSessionStore.create(session.did, sessionTtlSeconds);
